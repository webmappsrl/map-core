...
export class WmMapPositionDirective extends WmMapBaseDirective
  ...
  private _focus$: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
    false
  );
  ...
  constructor(@Host() mapCmp: WmMapComponent) {
    super(mapCmp);
    this.mapCmp.isInit$.pipe(filter(f => f === true), take(1)).subscribe(() => {
  ...
  */
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (
      changes.wmMapPositionfocus &&
      changes.wmMapPositionfocus.currentValue != null
    ) {
      ...
      if (val === true) {
        ...
        this.mapCmp.map
          .getView()
          .setZoom(this.mapCmp.map.getView().getZoom() + 1);
        this._setPositionByLocation(this._currentLocation);
      } else {
        this._layerLocation.setStyle(
          new Style({
  ...
  private _runningAvg(bearing: number): number {
    try {
      if (
        ...
        const runningAverage = Math.atan2(
          sinAverage / count,
          cosAverage / count
        );

        return runningAverage < 0
          ? Math.PI * 2 + runningAverage
          : runningAverage;
      }
      return 0;
    } catch (e) {
  ...
  */
  private _updateGeometry(loc: Location): Point {
    ...
    const geometry = circularPolygon(
